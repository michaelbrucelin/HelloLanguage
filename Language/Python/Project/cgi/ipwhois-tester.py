...
if len(ips) > 0:
    # 国家代码信息来自：https://zh.wikipedia.org/wiki/%E5%9C%8B%E5%AE%B6%E5%9C%B0%E5%8D%80%E4%BB%A3%E7%A2%BC
    ...
    for ip in ips:
        try:
            ...
            if rds.exists(ip):
                ...
                country_set = set([country_dict.get(re_country.match(s).group(1).upper(), re_country.match(s).group(1).upper()) for s in rows if re_country.match(s)])
                country_set.discard('ZZ')  # https://en.wikipedia.org/wiki/ISO_3166-1，ZZ等代码可以删除，实际只见到了ZZ，这里就只处理了ZZ
