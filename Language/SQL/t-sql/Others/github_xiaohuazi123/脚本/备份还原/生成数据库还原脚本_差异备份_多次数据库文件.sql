...
DECLARE @tbDatabaseFields  TABLE ([file_id] INT,
    [type] INT,
...
BEGIN
    ...
    INSERT @tbDatabaseFields
        ([file_id], [type], [logical_name], [physical_name], [backup_name])
    EXEC (@SQL)
...
BEGIN
    ...
    BEGIN
        ...
        DECLARE Cur_Database_Files CURSOR LOCAL FORWARD_ONLY FOR
            SELECT *
        FROM @tbDatabaseFields
        WHERE [backup_name]=@DbName
