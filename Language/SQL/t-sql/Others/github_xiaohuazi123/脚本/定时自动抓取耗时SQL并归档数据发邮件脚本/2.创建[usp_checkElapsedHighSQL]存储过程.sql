...
--创建存储过程
CREATE  PROCEDURE [dbo].[usp_checkElapsedHighSQL]
    (
    @SessionID INT
)
AS
BEGIN
    ...
    IF  ( SELECT OBJECT_ID('MonitorElapsedHighSQL.dbo.ElapsedHigh') ) IS NOT NULL
        ...
        SELECT @Duration = 10000
        --★Do -- in milliseconds, 10000 = 10 sec



        IF OBJECT_ID('tempdb..#ElapsedHigh') IS NOT NULL
                    BEGIN
            DROP TABLE [#ElapsedHigh]
        --删除临时表  
        END
        ...
        IF ( @SessionID IS NOT NULL AND @SessionID <> 0 )
            ...
            SELECT @stmttext = [text]
            FROM sys.fn_get_sql(@plan_handle)
