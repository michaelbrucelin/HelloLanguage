...
BEGIN
    ...
    IF ( ( SELECT OBJECT_ID('MonitorElapsedHighSQL.dbo.SQLCountStatisticsByDay') ) IS NULL
        AND ( SELECT OBJECT_ID('MonitorElapsedHighSQL.dbo.MostElapsedStatisticsByDay')) IS NULL
        AND ( SELECT OBJECT_ID('MonitorElapsedHighSQL.dbo.MostIOReadStatisticsByDay')) IS NULL
        AND ( SELECT OBJECT_ID('MonitorElapsedHighSQL.dbo.MostIOWriteStatisticsByDay')) IS NULL
        AND ( SELECT OBJECT_ID('MonitorElapsedHighSQL.dbo.sp_executesqlCountStatisticsByDay')) IS NULL
        )
            BEGIN
        RETURN 1
    ...
    END
        ...
        FROM ( SELECT ROW_NUMBER() OVER ( PARTITION BY spid ORDER BY [ElapsedMS] DESC ) rowid ,
                *
            FROM [ElapsedHigh]
            WHERE     [DBName] NOT IN ('MASTER','MODEL', 'MSDB', 'ReportServer', 'ReportServerTempDB', 'distribution')
                                ) t
        WHERE   rowid = 1
        ...
        FROM ( SELECT ROW_NUMBER() OVER ( PARTITION BY spid ORDER BY [IOWrites] DESC ) rowid ,
            ...
            WHERE     [planstmttext] LIKE '(@%'
                AND [DBName] NOT IN ( 'MASTER', 'MODEL', 'MSDB', 'ReportServer', 'ReportServerTempDB', 'distribution' )
                                ) t
        WHERE   rowid = 1
