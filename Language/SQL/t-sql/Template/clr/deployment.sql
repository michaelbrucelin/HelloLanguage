...
IF OBJECT_ID('dbo.fn_StringAgg') IS NOT NULL BEGIN
    DROP AGGREGATE dbo.fn_StringAgg
END;
IF OBJECT_ID('dbo.fn_ProductAgg') IS NOT NULL BEGIN
    DROP AGGREGATE dbo.fn_ProductAgg
END;
GO
--卸载当前的程序集
IF EXISTS(select *
...
--实现clr正则表达式提取模式匹配
IF OBJECT_ID('dbo.fn_RegExMatch') IS NOT NULL BEGIN
...
--实现clr正则表达式提取模式匹配——组
IF OBJECT_ID('dbo.fn_RegExMatchGroups') IS NOT NULL BEGIN
    DROP FUNCTION dbo.fn_RegExMatchGroups
END;
GO
...
--实现clr TrimStart
IF OBJECT_ID('dbo.fn_TrimStart') IS NOT NULL BEGIN
    DROP FUNCTION dbo.fn_TrimStart
END;
GO
...
--实现clr TrimEnd
IF OBJECT_ID('dbo.fn_TrimEnd') IS NOT NULL BEGIN
...
--实现clr Trim
IF OBJECT_ID('dbo.fn_Trim') IS NOT NULL BEGIN
...
IF OBJECT_ID('dbo.fn_SplitCLR') IS NOT NULL BEGIN
...
IF OBJECT_ID('dbo.fn_SplitByFlag') IS NOT NULL BEGIN
    DROP FUNCTION dbo.fn_SplitByFlag
END;
GO
...
IF OBJECT_ID('dbo.fn_SplitByChunk') IS NOT NULL BEGIN
    DROP FUNCTION dbo.fn_SplitByChunk
END;
GO
...
IF OBJECT_ID('dbo.fn_RandomStr') IS NOT NULL BEGIN
    DROP FUNCTION dbo.fn_RandomStr
END;
GO
...
--实现随机整型
--sql server调用时可以使用ABS(CHECKSUM(NEWID()))或CHECKSUM(NEWID())作为seed参数传入
IF OBJECT_ID('dbo.fn_RandomInt') IS NOT NULL BEGIN
...
--实现在指定的字符串列表中随机出一项
--sql server调用时可以使用ABS(CHECKSUM(NEWID()))或CHECKSUM(NEWID())作为seed参数传入
IF OBJECT_ID('dbo.fn_RandomItem') IS NOT NULL BEGIN
    DROP FUNCTION dbo.fn_RandomItem
END;
GO
...
--sql server调用时可以使用ABS(CHECKSUM(NEWID()))或CHECKSUM(NEWID())作为seed参数传入
IF OBJECT_ID('dbo.fn_RandomKey') IS NOT NULL BEGIN
    DROP FUNCTION dbo.fn_RandomKey
END;
GO
...
IF OBJECT_ID('dbo.fn_MapReplace') IS NOT NULL BEGIN
    DROP FUNCTION dbo.fn_MapReplace
END;
GO
...
IF OBJECT_ID('dbo.fn_MapReplace2') IS NOT NULL BEGIN
    DROP FUNCTION dbo.fn_MapReplace2
END;
GO
...
IF OBJECT_ID('dbo.fn_DecimalTo') IS NOT NULL BEGIN
    DROP FUNCTION dbo.fn_DecimalTo
END;
GO
...
IF OBJECT_ID('dbo.fn_DecimalFrom') IS NOT NULL BEGIN
    DROP FUNCTION dbo.fn_DecimalFrom
END;
GO
