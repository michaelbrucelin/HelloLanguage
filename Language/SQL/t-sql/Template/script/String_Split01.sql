...
set @str = '(values (''' + REPLACE(@str,@sep,''' ),( ''') + ''')) as tb(item)'
...
select SUBSTRING(c0.s, c1.n, CHARINDEX(@sep, c0.s+@sep, c1.n)-c1.n) as item,
       c1.n - LEN(REPLACE(LEFT(c0.s, c1.n), @sep, '')) + 1 as pos1,
       ROW_NUMBER() over(order by c1.n) as pos2,
       c1.n, c0.s
from c0
inner join c1 on c1.n <= LEN(c0.s) and SUBSTRING(@sep+c0.s, c1.n, 1) = @sep
