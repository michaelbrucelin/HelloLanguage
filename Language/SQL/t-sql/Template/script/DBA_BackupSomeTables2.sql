--此脚本用于将数据库中部分表从一个数据库迁移到另一个数据库，这里演示将2020年的表迁移到另一个数据库

create database [QXHReport_2020] CONTAINMENT = NONE
    on primary ( name = N'QXHReport_2020', filename = N'E:\Database\QXHReport DATA\QXHReport_2020.mdf' , size = 102400KB , filegrowth = 102400KB )
    log on ( name = N'QXHReport_2020_log', filename = N'E:\Database\QXHReport DATA\QXHReport_2020_log.ldf' , size = 1024KB , filegrowth = 102400KB )
go
alter database [QXHReport_2020] set recovery simple with no_wait
go
...
begin
    ...
    -- 创建目标表
    set @sql_c_table = N'select * into [QXHReport_2020].[dbo].['+@table_src+N']
    ...
    select @sql_d_index = (
    select 'drop index [' + ix.name + '] on [' + OBJECT_SCHEMA_NAME(ix.object_id) + '].[' + OBJECT_NAME(ix.object_id) + ']; '
    from sys.indexes as ix
    inner join sys.objects as tb on tb.object_id = ix.object_id
    ...
    begin
        begin tran
            ...
            output deleted.* into [QXHReport_2020].['+@schema_src+N'].['+@table_src+N']
            if @@rowcount < 100000
            begin
                commit tran
                break;
    ...
    begin
        dbcc SHRINKFILE (N'QXHReport' , 0, TRUNCATEONLY)
        set @movecnt = 0
    end
    fetch next from c into @db_src, @schema_src, @table_src
end
