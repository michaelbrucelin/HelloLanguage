	...
	public static void main(String[] args) {
		...
		try {
			...
			while (true) {
				...
				if (packet == null) {// 如果没收到数据包就继续下次搜索
				} else {
					i++;
					...
					if (msg[k].startsWith("\"MBX")) {//软交换前一个是flowno
						flown = msg[k - 1];
					} else {
						k++;
						while (k < length) {// 软交换前一个是flowno
					...
					if (i % 10000 == 0) {
						System.out.println("回复客户端：" + "*****" + clientAddress
								+ "***" + feedback);
...
class DoInsert implements Runnable {// 新的线程执行插入
	...
	public void run() {
		...
		while (true) {
			...
			if (Messcollection.mess.size() > rowsbatch) {// 存在一次插入的条数时，进行插入
				System.out.println(" 当前共有记录行数" + Messcollection.mess.size()
						+ "***" + TimeUtil.getCurrTimeS());
				Vector vtemp = new Vector();
				for (i = 0; i < rowsbatch; i++) {

					String element = Messcollection.mess.elementAt(i)
							.toString();
...
//				sbu.toString();
				times++;
				System.out.println(" 处理完毕还剩" + Messcollection.mess.size()
						+ "***" + TimeUtil.getCurrTimeS());
				System.out.println(" 收集数据 共" + i + "行 " + " times:" + times);
				// for循环结束 表明凑够了固定条数，开启插入工具

			}
			if (times == 3) // 凑够 n 轮插入一次
			{
				System.out.println("开始插入" + batchi++ + "批" + "***"
						+ TimeUtil.getCurrTimeS());
				MysqlJdbc
						.insertbatch("insert into udptest(callere164,calleraccesse164, calleee164,calleeaccesse164,callerip, "
								...
								// +
								// " ('Q0002CS100319607819993','9607819993','S0099966507944325','9001966507944325','198.154.109.36','CS10031_ZX','VOS3000 V2.1.3.2','9607819993',1,'213.230.188.1','ZX-00-05343_966ALL_DZS0099','','966507944325',1,1,5,0,1469683084946,1469683084946,10000,1988,0,'',0,0.0,0.0,0,0.0,'CS10031_ZX','Shaun Telecom Limited','',0.0,0.0,0,'P00-05343','Telenor',5166923438,'MBX5000_5:6288012166477876591,MBX5000_3:6288012081595114378',"
								// +
			...
			} else {// 没做插入 只是简单收集 间歇5秒 ，如果有插入动作，则间歇减小 变为2秒
