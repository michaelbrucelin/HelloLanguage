        ...
        try {
            ...
            for (int j = 1; j <= comlist.size() - lastLength; j++) {
                ...
                if (j % 100 == 0) {
                    ...
                    stmt = connect.prepareStatement(" into " + casTableName + "(" + column + ") VALUES "
                            + stbd.substring(0, stbd.length() - 1));
            ...
            for (int k = theStartPos; k < comlist.size(); k++) {
                temp = ("INSERT  into " + casTableName + "(" + column + ") VALUES (" + comlist.elementAt(k) + ")");
